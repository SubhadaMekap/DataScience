library(dplyr)
bankdata_train = read.csv(file.choose('bank-full_train.csv'), stringsAsFactors = F)
bankdata_test = read.csv(file.choose('bank-full_test.csv'), stringsAsFactors = F)
bankdata_test$y = NA
#mean(bankdata_train$age)
#outlier<-boxplot.stats(bankdata_train$balance)$out
#mo<-sum(outlier)
#mo
#variance_balance<-var(bankdata_train$balance)
#variance_balance
#library(mice)
#missing_values<-md.pattern(bankdata_train)
#library(outliers)
#utlier(bankdata_train$balance,opposite=TRUE)
bankdata_test$data = 'test'
bankdata_train$data = 'train'
#sum_Y_yes<-sum(bankdata_train$y=='yes')
#sum_Y_yes
#sum_Y_no<-sum(bankdata_train$y=='no')
#sum_Y_no
...
getwd()
write.csv(test.predicted, 'Bank_Marketing_Prediction.csv', row.names = FALSE)
